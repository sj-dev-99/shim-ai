...
import { LOVE_TEST_NAME, loveOptions, loveQuestions } from "../lib/loveTest";

export default function LoveTestPage() {
  const router = useRouter();
  const [answers, setAnswers] = useState<number[]>(Array(loveQuestions.length).fill(0));
  const [current, setCurrent] = useState(0);
  const selected = answers[current];
  const progress = Math.round(((current + 1) / loveQuestions.length) * 100);
  ...
  function goNext() {
    if (current < loveQuestions.length - 1) {
      setCurrent((item) => item + 1);
      return;
    }

    router.push(`/love-result?answers=${answers.join(",")}`);
  }

  function reset() {
    setAnswers(Array(loveQuestions.length).fill(0));
    setCurrent(0);
  }

  const canFinish = answeredCount === loveQuestions.length;

  return (
    <>
      <Head>
        <title>테스트 | {LOVE_TEST_NAME}</title>
      </Head>
      <main className="page-shell">
        ...
        <section className="progress-row" aria-label="진행률">
          <div className="progress-meta">
            <span>
              {current + 1} / {loveQuestions.length}
            </span>
            <span>{progress}%</span>
          </div>
        ...
        <section className="question-panel">
          <h1>{loveQuestions[current].text}</h1>
          ...
          <div className="test-controls">
            ...
            <button
              ...
              disabled={current === loveQuestions.length - 1 ? !canFinish : selected === 0}
              onClick={goNext}
              type="button"
            >
              {current === loveQuestions.length - 1 ? "결과 보기" : "다음"}
