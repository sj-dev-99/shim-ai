...
SHIM Diary는 사용자가 오늘의 감정과 경험을 기록하는 공간입니다. 감정 상태를 선택하고 일기 형태로 내용을 남기면, 현재 구현에서는 내부 로직을 통해 AI 한 줄 코멘트 형태의 피드백을 제공합니다.

로그인한 사용자의 Diary 기록은 Supabase에 계정별로 저장됩니다. 비로그인 상태에서는 저장 대신 로그인 안내와 임시 draft 보존 흐름을 사용합니다. 기존 브라우저 `localStorage` 기록은 로그인 후 가져오기 기능으로 Supabase 기록에 옮길 수 있습니다.
